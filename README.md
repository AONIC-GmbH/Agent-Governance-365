...
Power Platform inventory and asset management. Monorepo with a React
(Vite) frontend and an Express API, using **Microsoft Entra ID** for sign-in and
**PostgreSQL** (Azure or local) for persistence.
...
Jobs can be started from Admin UI or via a secret-gated internal endpoint used
by a nightly timer (Logic App / Azure Function).
...
You typically need **two** (optionally three) app registrations in **your**
tenant.

### 2.1 SPA (frontend login)

1. Azure Portal → Microsoft Entra ID → App registrations → **New registration**
2. Name e.g. `runpipe-spa`
...
1. App registration e.g. `runpipe-pp-inventory`
...
### 2.5 Copilot Agent Kit usage (optional)

Inventory does **not** come from the Power Platform CoE Starter Kit. Copilot
credit usage is read from the Agent Kit Dataverse table
...
11. [ ] (Optional) Power BI and/or Copilot Agent Kit credentials  
...
[MIT](LICENSE). Copyright AONIC GmbH.
